...
class KNN:
    ...
    def compute_distances_two_loops(self, X):
        ...
        for i_test in range(num_test):
            for i_train in range(num_train):
                ...
                dists[i_test][i_train] = np.sum(np.abs(self.train_X[i_train] -
                                                       X[i_test]))
        return dists
    ...
    def compute_distances_one_loop(self, X):
        ...
        for i_test in range(num_test):
            ...
            dists[i_test] = np.sum(np.abs(self.train_X[:] - X[i_test]),
                                   axis=1)
        return dists
    ...
    def predict_labels_binary(self, dists):
        ...
        for i in range(num_test):
            ...
            nearest_k = self.train_y[np.argsort(dists[i])[:self.k]]
            pred[i] = (nearest_k.mean() > 0.5)
